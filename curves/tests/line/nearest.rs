...
fn nearest_x_axis_aligned_1() {
    ...
    assert!(
        x_line
            .nearest_point(&Coord2(100.0, 20.0))
            .distance_to(&Coord2(100.0, 0.0))
            < 0.01
    );
}
...
fn nearest_origin() {
    ...
    let perpendicular_line = (
        Coord2(4.0, 5.0),
        Coord2(4.0 - (9.0 - 5.0), 5.0 + (7.0 - 4.0)),
    );
    ...
    assert!(
        origin_nearest
            .distance_to(&line.point_at_pos(nearest_t_value_iteration(&line, &from_origin)))
            < 0.1
    );
    assert!(origin_nearest.distance_to(&Coord2(4.0, 5.0)) < 0.01);
}
...
fn nearest_2() {
    ...
    let perpendicular_line = (
        point,
        Coord2(point.x() - (9.0 - 5.0), point.y() + (7.0 - 4.0)),
    );
    ...
    assert!(
        point_nearest
            .distance_to(&line.point_at_pos(nearest_t_value_iteration(&line, &from_point)))
            < 0.1
    );
