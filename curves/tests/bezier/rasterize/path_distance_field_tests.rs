...
fn corners_are_outside() {
    ...
    assert!(
        circle_field.distance_at_point(ContourPosition(0, 0)) > 0.0,
        "Distance at 0,0 is {:?}",
        circle_field.distance_at_point(ContourPosition(0, 0))
    );
    assert!(
        circle_field.distance_at_point(ContourPosition(999, 0)) > 0.0,
        "Distance at 999,0 is {:?}",
        circle_field.distance_at_point(ContourPosition(999, 0))
    );
    assert!(
        circle_field.distance_at_point(ContourPosition(0, 999)) > 0.0,
        "Distance at 0,999 is {:?}",
        circle_field.distance_at_point(ContourPosition(0, 999))
    );
    assert!(
        circle_field.distance_at_point(ContourPosition(999, 999)) > 0.0,
        "Distance at 999,999 is {:?}",
        circle_field.distance_at_point(ContourPosition(999, 999))
    );
}
...
fn center_is_inside() {
    ...
    println!(
        "{:?}",
        circle_field.distance_at_point(ContourPosition(501, 501))
    );

    assert!(
        circle_field.distance_at_point(ContourPosition(501, 501)) < 0.0,
        "Distance at center is {:?}",
        circle_field.distance_at_point(ContourPosition(501, 501))
    );
    assert!(
        circle_field.distance_at_point(ContourPosition(500, 500)) < 0.0,
        "Distance near center is {:?}",
        circle_field.distance_at_point(ContourPosition(500, 500))
    );
    assert!(
        circle_field.distance_at_point(ContourPosition(499, 499)) < 0.0,
        "Distance near center is {:?}",
        circle_field.distance_at_point(ContourPosition(499, 499))
    );
}
...
fn outside_point_distances() {
    ...
    for y in 0..1000 {
        for x in 0..1000 {
            ...
            assert!(
                (field_distance - (to_center - 300.0)).abs() < 2.0,
                "Distance at {}, {} is {} ({} to center)",
                x,
                y,
                field_distance,
                to_center
            ...
            if field_distance < 0.0 && field_distance >= -0.05 {
                ...
                assert!(
                    field_distance >= 0.0,
                    "Distance at {}, {} is {} (this point is not inside the circle)",
                    x,
                    y,
                    field_distance
                );
...
fn inside_point_distances() {
    ...
    for y in 0..1000 {
        for x in 0..1000 {
            ...
            assert!(
                field_distance <= 0.0,
                "Distance at {}, {} is {} (this point is not outside the circle)",
                x,
                y,
                field_distance
            );
...
fn nearby_point_distances() {
    ...
    let circle_field =
        PathDistanceField::from_path(vec![circle_path.clone()], ContourSize(1000, 1000));
...
fn trace_circle() {
    ...
    for curve in traced_circle[0].to_curves::<Curve<_>>() {
        for t in 0..100 {
            ...
            let nearest_distance = circle_path
                .to_curves::<Curve<_>>()
                .into_iter()
...
fn trace_chisel_paths() {
    ...
    debug_assert!(
        max_error < 0.4,
        "Max error was {:?} (average {:?})",
        max_error,
        avg_error
    );
    debug_assert!(
        traced_chisel[0].to_curves::<Curve<_>>().len() < 16,
        "Result has {} curves",
        traced_chisel[0].to_curves::<Curve<_>>().len()
    );
}
