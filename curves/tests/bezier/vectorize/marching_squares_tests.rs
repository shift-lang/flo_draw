...
fn double_loops() {
    ...
    let contour = U8SampledContour(ContourSize(3, 3), vec![1, 0, 0, 0, 0, 0, 0, 0, 1]);
...
fn triple_loops() {
    ...
    let contour = U8SampledContour(ContourSize(3, 3), vec![1, 0, 0, 0, 1, 0, 0, 0, 1]);
...
fn circle_points_from_contours() {
    ...
    let circle = circle[0]
        .iter()
        .map(|edge| edge.to_coords::<Coord2>(ContourSize(size, size)))
        .collect::<Vec<_>>();
...
fn circle_edges_from_contours() {
    ...
    let circle = circle[0]
        .iter()
        .map(|edge| edge.to_contour_coords(ContourSize(size, size)))
        .collect::<Vec<_>>();
    ...
    for (from, to) in circle {
        ...
        if (from_inside && to_inside) || (!from_inside && !to_inside) {
            all_edges = false;
            println!(
                "Not an edge {:?} {:?} ({:?}-{:?})",
                from, to, from_inside, to_inside
            );
        }
    }
...
fn circle_path_from_distance_field() {
    ...
    assert!(circle.len() == 1, "{:?}", circle);
    assert!(
        circle[0].to_curves::<Curve<_>>().len() < 10,
        "Path has {} curves",
