    ...
    C::Point: Coordinate2D,
{
    ...
    let control_point_to_point = curve_points
        .iter()
        .map(|control_point| *control_point - point)
        .collect::<SmallVec<[_; 4]>>();
    let control_point_to_next = curve_points
        .iter()
        .tuple_windows()
    ...
    let cp_dot_products = control_point_to_next
        .into_iter()
        .map(|to_next_cp| {
            control_point_to_point
                .iter()
    ...
    for k in 0..=5i32 {
        ...
        for i in lower..=upper {
            ...
            curve[(i + j) as usize].1 +=
                cp_dot_products[j as usize][i as usize] * Z[j as usize][i as usize];
        }
    }
    ...
    C::Point: Coordinate + Coordinate2D,
{
    ...
    for t in perpendicular_t_values
        .into_iter()
