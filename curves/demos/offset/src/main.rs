...
fn main() {
    with_2d_graphics(|| {
        ...
        loop {
            ...
            // Borrowing the animation from https://www.shadertoy.com/view/4sKyzW because it's interesting
            // (and because I'm interested in using distance fields to make an improved offset algorithm)
            ...
            let offset_curve_2 = bezier::offset_lms_subdivisions(
                &initial_curve,
                |t| -((off2 - off1) * t + off1),
                |_| 0.0,
                &bezier::SubdivisionOffsetOptions::default(),
            )
            .unwrap();
            // let offset_curve_3 = bezier::offset_lms_sampling(
            //     &initial_curve,
            //     |t| ((off2 - off1) * t + off1) * (t * 32.0).cos(),
            //     |_| 0.0,
            //     200,
            //     1.0,
            // )
            ...
            let stroked_curve = stroke_path::<SimpleBezierPath, _>(
                &curve_path,
                10.0,
            ...
            // Render to a path
            let offset_curve_3 = brush_stroke_from_curve::<SimpleBezierPath, _, _>(
                &CircularBrush,
                &brush_curve,
                0.5,
                0.25,
            ...
            canvas.draw(|gc| {
                ...
                gc.new_path();
                gc.move_to(
                    initial_curve.start_point().x() as _,
                    initial_curve.start_point().y() as _,
                );
                ...
                gc.new_path();
                gc.move_to(
                    offset_curve_1[0].start_point().x() as _,
                    offset_curve_1[0].start_point().y() as _,
                );
                ...
                gc.new_path();
                gc.move_to(
                    offset_curve_2[0].start_point().x() as _,
                    offset_curve_2[0].start_point().y() as _,
                );
                ...
                for subpath in offset_curve_3.iter() {
                    ...
                    gc.move_to(
                        curve[0].start_point().x() as _,
                        curve[0].start_point().y() as _,
                    );
                    for c in curve.iter() {
                        let (cp1, cp2) = c.control_points();
                        let end = c.end_point();
                ...
                for curve in
                    vec![&vec![initial_curve], &offset_curve_1, &offset_curve_2].into_iter()
                {
                    gc.line_width(1.0);
