    ...
    fn wake_with(&self, new_waker: task::Waker) {
        ...
        let to_wake = {
            ...
            match state {
                ...
                NotWoken => {
                    *new_state = WillWakeWithWaker(new_waker);
                    None
                }
                WillWakeWithWaker(_old_waker) => {
                    *new_state = WillWakeWithWaker(new_waker);
                    None
                }
            }
...
impl<T> FutureResultState<T> {
    ...
    fn take(&mut self) -> Option<T> {
        ...
        match new_value {
            FutureResultState::None => {
                *self = FutureResultState::None;
                None
            }
            FutureResultState::ReturnedViaFuture => {
