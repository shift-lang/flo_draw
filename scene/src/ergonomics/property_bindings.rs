...
pub trait SceneContextPropertiesExt {
    ...
    ///
    fn property_create_on_entity<TValue>(
        &self,
        entity_id: EntityId,
        property_name: &str,
        binding: impl Into<BindRef<TValue>>,
    ...
    ///
    fn property_bind<TValue>(
        &self,
        entity_id: EntityId,
        property_name: &str,
    ) -> BoxFuture<'static, Result<BindRef<TValue>, EntityChannelError>>
    where
    ...
    ///
    fn rope_create<TCell, TAttribute>(
        &self,
        property_name: &str,
        binding: impl Into<RopeBinding<TCell, TAttribute>>,
    ) -> BoxFuture<'static, Result<(), EntityChannelError>>
    where
    ...
    /// Creates a property from a `flo_binding` binding on a different entity
    ///
    fn rope_create_on_entity<TCell, TAttribute>(
        &self,
        entity_id: EntityId,
        property_name: &str,
        binding: impl Into<RopeBinding<TCell, TAttribute>>,
    ...
    /// Creates a binding from a known entity ID and property name
    ///
    fn rope_bind<TCell, TAttribute>(
        &self,
        entity_id: EntityId,
        property_name: &str,
    ) -> BoxFuture<'static, Result<RopeBinding<TCell, TAttribute>, EntityChannelError>>
    where
        TCell: 'static + Send + Unpin + Clone + PartialEq,
        TAttribute: 'static + Send + Sync + Unpin + Clone + PartialEq + Default;
}
...
impl SceneContextPropertiesExt for Arc<SceneContext> {
        ...
        TValue: 'static + PartialEq + Clone + Send + Sized,
    {
        ...
        let property_definition = entity_id
            .map(|entity_id| PropertyDefinition::from_binding(entity_id, property_name, binding));
        ...
        async move {
            if let Some(property_definition) = property_definition {
                ...
                // Create the property
                channel
                    .send(PropertyRequest::CreateProperty(property_definition))
                    .await?;
        ...
        TValue: 'static + PartialEq + Clone + Send + Sized,
    {
        ...
        let property_definition =
            PropertyDefinition::from_binding(entity_id, property_name, binding);
        ...
        TValue: 'static + Clone + Send + PartialEq,
    {
        ...
        async move {
            ...
            channel
                .send(PropertyRequest::Get(reference, target))
                .await?;
        ...
        TAttribute: 'static + Send + Sync + Unpin + Clone + PartialEq + Default,
    {
        let context = Arc::clone(self);
        let entity_id = self.entity();
        let property_definition = entity_id.map(|entity_id| {
            RopePropertyDefinition::from_binding(entity_id, property_name, binding)
        });
        ...
        async move {
            if let Some(property_definition) = property_definition {
                // Retrieve the channel
                let mut channel =
                    rope_properties_channel::<TCell, TAttribute>(PROPERTIES, &context).await?;

                // Create the property
                channel
                    .send(RopePropertyRequest::CreateProperty(property_definition))
                    .await?;
        ...
        TAttribute: 'static + Send + Sync + Unpin + Clone + PartialEq + Default,
    {
        ...
        async move {
            ...
            let (binding, target) = FloatingBinding::new();
            channel
                .send(RopePropertyRequest::Get(reference, target))
                .await?;
...
/// Creates a property from a `flo_binding` binding on the current entity
///
pub async fn property_create<TValue>(
    property_name: &str,
    binding: impl Into<BindRef<TValue>>,
    ...
    TValue: 'static + Clone + Send + PartialEq,
{
    SceneContext::current()
        .property_create(property_name, binding)
        .await
}
...
/// Creates a property from a `flo_binding` binding on a different entity
///
pub async fn property_create_on_entity<TValue>(
    entity_id: EntityId,
    property_name: &str,
    binding: impl Into<BindRef<TValue>>,
) -> Result<(), EntityChannelError>
...
{
    SceneContext::current()
        .property_create_on_entity(entity_id, property_name, binding)
        .await
}
...
/// Creates a binding from a known entity ID and property name
///
pub async fn property_bind<TValue>(
    entity_id: EntityId,
    property_name: &str,
) -> Result<BindRef<TValue>, EntityChannelError>
where
    TValue: 'static + Clone + Send + PartialEq,
{
    SceneContext::current()
        .property_bind(entity_id, property_name)
        .await
}
...
/// Creates a rope property from a `RopeBinding` on the current entity
///
pub fn rope_create<TCell, TAttribute>(
    property_name: &str,
    binding: impl Into<RopeBinding<TCell, TAttribute>>,
) -> BoxFuture<'static, Result<(), EntityChannelError>>
where
...
/// Creates a property from a `flo_binding` binding on a different entity
///
pub fn rope_create_on_entity<TCell, TAttribute>(
    entity_id: EntityId,
    property_name: &str,
    binding: impl Into<RopeBinding<TCell, TAttribute>>,
) -> BoxFuture<'static, Result<(), EntityChannelError>>
...
/// Creates a binding from a known entity ID and property name
///
pub fn rope_bind<TCell, TAttribute>(
    entity_id: EntityId,
    property_name: &str,
) -> BoxFuture<'static, Result<RopeBinding<TCell, TAttribute>, EntityChannelError>>
where
