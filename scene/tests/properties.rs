    ...
    scene
        .create_entity(TEST_ENTITY, move |_context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                let same_channel =
                    properties_channel::<i64>(PROPERTIES, &SceneContext::current()).await;
    ...
    scene
        .create_entity(TEST_ENTITY, move |_context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                // Try to open the channel to the properties entity and ensure that it's there
                let channel =
                    properties_channel::<String>(PROPERTIES, &SceneContext::current()).await;
                let same_channel =
                    properties_channel::<String>(PROPERTIES, &SceneContext::current()).await;
    ...
    scene
        .create_entity(TEST_ENTITY, move |_context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                let mut channel =
                    properties_channel::<String>(PROPERTIES, &SceneContext::current())
                        .await
                        .unwrap();
                ...
                channel
                    .send(PropertyRequest::CreateProperty(
                        PropertyDefinition::from_stream(
                            TEST_ENTITY,
                            "TestString",
                            string_receiver.boxed(),
                            "".into(),
                        ),
                    ))
                    .await
                ...
                channel
                    .send(PropertyRequest::Get(
                        PropertyReference::new(TEST_ENTITY, "TestString"),
                        target,
                    ))
                    .await
                    .unwrap();
                ...
                msg.send((set_value == Some("Test".to_string())).into())
                    .await
                    .ok();
            }
        })
    ...
    scene
        .create_entity(TEST_ENTITY, move |_context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                property_create("TestProperty", binding.clone())
                    .await
                    .unwrap();

                // Retrieve the binding for the property we just created
                let value = property_bind::<String>(TEST_ENTITY, "TestProperty")
                    .await
                    .unwrap();
                ...
                msg.send((initial_value == "Test".to_string()).into())
                    .await
                    .ok();
                msg.send((initial_value_again == Some("Test".to_string())).into())
                    .await
                    .ok();
                msg.send((next_value == Some("AnotherTest".to_string())).into())
                    .await
                    .ok();
                msg.send((another_value == "AnotherTest".to_string()).into())
                    .await
                    .ok();
            }
        })
    ...
    scene
        .create_entity(TEST_ENTITY, move |_context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                rope_create("TestProperty", RopeBinding::from_mutable(&binding))
                    .await
                    .unwrap();

                // Retrieve the binding for the property we just created
                let value = rope_bind::<char, ()>(TEST_ENTITY, "TestProperty")
                    .await
                    .unwrap();
                ...
                msg.send((another_value == vec!['T', 'e', 's', 't']).into())
                    .await
                    .ok();
            }
        })
...
fn property_unbinds_when_entity_destroyed() {
    ...
    scene
        .create_entity(property_entity, move |_context, mut msg| async move {
            while let Some(msg) = msg.next().await {
    ...
    scene
        .create_entity(TEST_ENTITY, move |context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                // Create a string property from a binding
                let binding = bind("Test".to_string());
                property_create_on_entity(property_entity, "TestProperty", binding.clone())
                    .await
                    .unwrap();

                // Retrieve the binding for the property we just created
                let value = property_bind::<String>(property_entity, "TestProperty")
                    .await
                    .unwrap();
                let initial_value = value.get();
                ...
                let (stopped_entities, mut stop_receiver) =
                    SimpleEntityChannel::new(TEST_ENTITY, 5);
                context
                    .send(
                        ENTITY_REGISTRY,
                        EntityRegistryRequest::TrackEntities(stopped_entities.boxed()),
                    )
                    .await
                    .unwrap();
...
fn entities_property() {
    ...
    scene
        .create_entity(sample_entity, |_ctxt, mut messages| async move {
    ...
    scene
        .create_entity(TEST_ENTITY, move |_context, mut msg| async move {
            while let Some(msg) = msg.next().await {
                ...
                let entities = rope_bind::<EntityId, ()>(PROPERTIES, "Entities")
                    .await
                    .unwrap();
                ...
                loop {
                    ...
                    if entities
                        .read_cells(0..entities.len())
                        .any(|entity_id| entity_id == sample_entity)
                    {
                        break;
                    }
    ...
    scene
        .create_entity(TEST_ENTITY, move |context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                // Retrieve the binding for the property we just created
                let _value = property_bind::<String>(TEST_ENTITY, "TestProperty")
                    .await
                    .unwrap();

                // Request tracking information on the specified property
                let mut property_channel = properties_channel::<String>(PROPERTIES, &context)
                    .await
                    .unwrap();
                ...
                while let Some(PropertyUpdate::Created(property_reference)) =
                    track_strings.next().await
                {
    ...
    scene
        .create_entity(TEST_ENTITY, move |context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                property_channel
                    .send(PropertyRequest::DestroyProperty(PropertyReference::new(
                        TEST_ENTITY,
                        "TestProperty",
                    )))
                    .await
                ...
                while let Some(PropertyUpdate::Destroyed(property_reference)) =
                    track_strings.next().await
                {
                    if property_reference.name == Arc::new("TestProperty".into()) {
                        destroyed = true;
    ...
    scene
        .create_entity(TEST_ENTITY, move |context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                property_create("TestProperty", bind("Also test".to_string()))
                    .await
                    .unwrap();
    ...
    scene
        .create_entity(TEST_ENTITY, move |context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                // Create a string property from a binding
                let binding = bind("Test".to_string());
                property_create_on_entity(entity_id, "TestProperty", binding.clone())
                    .await
                    .unwrap();

                // Retrieve the binding for the property we just created
                let _value = property_bind::<String>(entity_id, "TestProperty")
                    .await
                    .unwrap();
    ...
    scene
        .create_entity(TEST_ENTITY, move |context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                property_create_on_entity(first_entity_id, "TestProperty", binding_1.clone())
                    .await
                    .unwrap();
                property_create_on_entity(second_entity_id, "TestProperty", binding_2.clone())
                    .await
                    .unwrap();
                ...
                if let FollowAll::NewValue(owner, value) = initial_1 {
                    if owner == first_entity_id {
                        ...
                        msg.send(SceneTestResult::FailedWithMessage(
                            "Was expecting a value for one of our two entities".to_string(),
                        ))
                        .await
                        .ok();
                        return;
                    ...
                    msg.send(SceneTestResult::FailedWithMessage(
                        "Was expecting a new value".to_string(),
                    ))
                    .await
                    .ok();
                    return;
                ...
                msg.send(
                    (new_value_1
                        == FollowAll::NewValue(first_entity_id, "New value 1".to_string()))
                    .into(),
                )
                .await
                .ok();
                ...
                msg.send(
                    (new_value_2
                        == FollowAll::NewValue(second_entity_id, "New value 2".to_string()))
                    .into(),
                )
                .await
                .ok();
                ...
                msg.send((second_entity_stopped == FollowAll::Destroyed(second_entity_id)).into())
                    .await
                    .ok();
                ...
                msg.send(
                    (new_value_3
                        == FollowAll::NewValue(first_entity_id, "New value 4".to_string()))
                    .into(),
                )
                .await
                .ok();
                ...
                msg.send((first_entity_stopped == FollowAll::Destroyed(first_entity_id)).into())
                    .await
                    .ok();
    ...
    scene
        .create_entity(TEST_ENTITY, move |context, mut msg| async move {
            ...
            while let Some(msg) = msg.next().await {
                ...
                property_channel
                    .send(PropertyRequest::Follow(
                        PropertyReference::new(TEST_ENTITY, "TestProperty"),
                        tracker_channel.boxed(),
                    ))
                    .await
                    .unwrap();
