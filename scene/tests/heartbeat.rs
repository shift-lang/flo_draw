...
fn receive_heartbeat_after_message() {
    ...
    scene
        .create_entity(receive_heartbeat, |_context, mut msg| async move {
    ...
    scene
        .create_entity(TEST_ENTITY, move |_context, mut msg| async move {
            ...
            scene_send(
                HEARTBEAT,
                HeartbeatRequest::RequestHeartbeat(receive_heartbeat_channel),
            )
            .await
            .unwrap();
            ...
            while let Some(msg) = msg.next().await {
                ...
                scene_send(receive_heartbeat, TestRequest::Message)
                    .await
                    .unwrap();
