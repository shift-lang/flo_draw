...
impl Draw {
    ...
    pub(crate) fn uses_resource(&self, resource: &DrawResource) -> bool {
        ...
        match self {
            DashLength(_) | DashOffset(_) => resource == &DrawResource::StrokeDash,

            // The fill and stroke operations depend on multiple resources, so their resource is 'special'
            Fill => match resource {
                DrawResource::CanvasTransform
                | DrawResource::FillWindingRule
                | DrawResource::FillBlend
                | DrawResource::FillColor => true,
                _ => false,
            },
            Stroke => match resource {
                DrawResource::CanvasTransform
                | DrawResource::StrokeLineWidth
                | DrawResource::StrokeLineCap
                | DrawResource::StrokeLineJoin
                | DrawResource::StrokeDash
                | DrawResource::StrokeColor
                | DrawResource::FillBlend => true,
                _ => false,
            },
            ...
            Font(font_id, FontOp::LayoutText(_)) | Font(font_id, FontOp::DrawGlyphs(_)) => {
                match resource {
                    DrawResource::Font(resource_font_id)
                    | DrawResource::FontSize(resource_font_id) => font_id == resource_font_id,
                    DrawResource::CanvasTransform
                    | DrawResource::FillWindingRule
                    | DrawResource::FillBlend
                    | DrawResource::FillColor => true,
            ...
            DrawSprite(sprite_id) => {
                resource == &DrawResource::CanvasTransform
                    || resource == &DrawResource::Sprite(*sprite_id)
            }

            // DrawText and FillTexture use the corresponding resource
            DrawText(font_id, _, _, _) => match resource {
                DrawResource::Font(resource_font_id) | DrawResource::FontSize(resource_font_id) => {
                    font_id == resource_font_id
                }
            ...
            CenterRegion(_, _) | MultiplyTransform(_) => resource == &DrawResource::CanvasTransform,

            SwapLayers(layer1, layer2) => {
                resource == &DrawResource::Layer(*layer1)
                    || resource == &DrawResource::Layer(*layer2)
            }
    ...
    ///
    #[inline]
    pub(crate) fn source_resource(
        &self,
        active_resource: &DrawResource,
    ) -> SmallVec<[DrawResource; 8]> {
        use self::Draw::*;
        ...
        match self {
            ...
            SwapLayers(layer1, layer2) => {
                smallvec![DrawResource::Layer(*layer1), DrawResource::Layer(*layer2)]
            }
            ...
            LineWidth(_) | LineWidthPixels(_) | LineJoin(_) | LineCap(_) | NewDashPattern
            | StrokeColor(_) => smallvec![],

            WindingRule(_) | BlendMode(_) | FillColor(_) => smallvec![],
            ...
            DashLength(_) | DashOffset(_) => smallvec![DrawResource::StrokeDash],
            ...
            // Texture and font operations generally alter the existing resource so they have a dependency
            Texture(texture_id, TextureOp::CreateDynamicSprite(sprite_id, _, _)) => smallvec![
                DrawResource::Texture(*texture_id),
                DrawResource::Sprite(*sprite_id),
                DrawResource::CanvasTransform
            ],
            ...
            Font(font_id, FontOp::LayoutText(_)) | Font(font_id, FontOp::DrawGlyphs(_)) => {
                smallvec![
                    *active_resource,
                    DrawResource::Font(*font_id),
                    DrawResource::FontSize(*font_id),
                    DrawResource::CanvasTransform,
                    DrawResource::FillWindingRule,
                    DrawResource::FillBlend,
                    DrawResource::FillColor
            ...
            DrawSprite(sprite_id) => smallvec![
                DrawResource::CanvasTransform,
                DrawResource::Sprite(*sprite_id)
            ],

            // DrawText and FillTexture use the corresponding resource
            DrawText(font_id, _, _, _) => smallvec![
                *active_resource,
                DrawResource::CanvasTransform,
                DrawResource::Font(*font_id),
                DrawResource::FontSize(*font_id)
            ...
            // Transforms use the 'canvas' resource (setting the height or the identity transform resets any previous transform)
            IdentityTransform | CanvasHeight(_) => smallvec![],

            CenterRegion(_, _) | MultiplyTransform(_) => smallvec![DrawResource::CanvasTransform],
    ...
    pub(crate) fn target_resource(&self, active_resource: &DrawResource) -> DrawResource {
        ...
        match self {
            StartFrame | ShowFrame | ResetFrame => DrawResource::Frame,

            ClearCanvas(_) => DrawResource::Canvas,
            IdentityTransform | CanvasHeight(_) | CenterRegion(_, _) | MultiplyTransform(_) => {
                DrawResource::CanvasTransform
            }

            Layer(layer_id) => DrawResource::Layer(*layer_id),

            LineWidth(_) | LineWidthPixels(_) => DrawResource::StrokeLineWidth,
            LineJoin(_) => DrawResource::StrokeLineJoin,
            LineCap(_) => DrawResource::StrokeLineCap,
            NewDashPattern | DashLength(_) | DashOffset(_) => DrawResource::StrokeDash,
            StrokeColor(_) => DrawResource::StrokeColor,

            WindingRule(_) => DrawResource::FillWindingRule,
            BlendMode(_) => DrawResource::FillBlend,
            FillColor(_) | FillGradient(_, _, _) | FillTexture(_, _, _) | FillTransform(_) => {
                DrawResource::FillColor
            }
