    ...
    Attribute: 'static + Send + Sync + Clone + Unpin + PartialEq + Default,
{
    ...
    ///
    fn map<NewCell, MapFn: 'static + Send + Fn(Cell) -> NewCell>(
        &self,
        map_fn: MapFn,
    ...
    TRope: BoundRope<Cell, Attribute>,
{
    fn chain<OtherRope: BoundRope<Cell, Attribute>>(
        ...
        let concat_stream = stream::poll_fn(move |ctxt| {
            ...
            let poll_right = follow_right
                .as_mut()
                .map(|right| right.poll_next_unpin(ctxt));
        ...
        NewCell: 'static + Send + Unpin + Clone + PartialEq,
    {
        ...
        let mapped_stream = stream::poll_fn(move |ctxt| {
            ...
            match changes.poll_next_unpin(ctxt) {
                ...
                Poll::Ready(Some(Replace(range, cells))) => Poll::Ready(Some(Replace(
                    range,
                    cells.into_iter().map(&map_fn).collect(),
                ))),
                Poll::Ready(Some(SetAttributes(range, attributes))) => {
                    Poll::Ready(Some(SetAttributes(range, attributes)))
                }
                Poll::Ready(Some(ReplaceAttributes(range, cells, attributes))) => {
                    Poll::Ready(Some(ReplaceAttributes(
                        range,
                        cells.into_iter().map(&map_fn).collect(),
                        attributes,
                    )))
